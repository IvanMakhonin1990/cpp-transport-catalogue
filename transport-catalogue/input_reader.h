...
std::pair<std::string, Coordinates>
ParseStop(const Request &request,
          std::vector<std::tuple<std::string, std::string, double>> &distances);


std::pair<std::string, std::vector<std::string_view>> ParseBus(const Request& request);

std::vector<std::string_view> SplitIntoWords(std::string_view text, const char& symbol = ' ');
